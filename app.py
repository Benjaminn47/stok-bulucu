        ...
        if not GEMINI_API_KEY or not TAVILY_API_KEY:
            ...
            with st.spinner("Yapay zeka görseli analiz ediyor ve canlı stok taraması yapıyor..."):
                try:
                    ...
                    # AKILLI KİMLİK DOĞRULAMA AYARI (401 Hatalarını Önler)
                    headers = {"Content-Type": "application/json"}
                    
                    # Eğer şifre AQ. ile başlıyorsa Bearer Token olarak geçiriyoruz
                    if GEMINI_API_KEY.startswith("AQ."):
                        url = "https://generativelanguage.googleapis.com/v1beta/models/gemini-1.5-flash:generateContent"
                        headers["Authorization"] = f"Bearer {GEMINI_API_KEY}"
                    else:
                        # Eğer eski tip AIzaSy ile başlıyorsa standart URL parametresi olarak geçiriyoruz
                        url = f"https://generativelanguage.googleapis.com/v1beta/models/gemini-1.5-flash:generateContent?key={GEMINI_API_KEY}"
                    
                    prompt = "Bu görseldeki ürünün tam markasını, model numarasını ve rengini kısa bir metin olarak yaz."
                    
                    # Tek bir satırda, hatasız ve asla kırpılmayacak akıllı JSON veri paketi
                    payload = {"contents":}]}
                    ...
                    if response.status_code!= 200:
                        ...
                        # 2. Aşama: Tavily API ile Derinlemesine Web ve İndirim Araması (Sadece başarılı tespit durumunda çalışır)
                        ...
                        for kupon in kupon_sonuclari['results']:
                            st.markdown(f"* **[Kupon Kaynağı: {kupon['title']}]({kupon['url']})**\n> {kupon['content']}")
