        ...
        if not GEMINI_API_KEY or not TAVILY_API_KEY:
            ...
            with st.spinner("Yapay zeka görseli analiz ediyor ve canlı stok taraması yapıyor..."):
                try:
                    ...
                    prompt = "Bu görseldeki ürünün tam markasını, model numarasını ve rengini kısa bir metin olarak yaz. Sadece ürün bilgisini ver."
                    
                    # DÜZELTİLMİŞ ve DOĞRU PAYLOAD
                    payload = {
                        "contents": [{
                            "parts": [{
                                "text": prompt,
                    ...
                    if response.status_code != 200:
                        ...
                        try:
                            ...
                            st.warning("Gemini'den net bir ürün bilgisi alınamadı.")
                            st.json(response_json)
                    
                    # 2. Tavily ile arama
                    if tespit_edilen_urun and tespit_edilen_urun != "Ürün tespit edilemedi.":
                        ...
                        stok_sonuclari = tavily_client.search(arama_sorgusu, search_depth="advanced", max_results=5)
                        kupon_sonuclari = tavily_client.search(indirim_sorgusu, search_depth="basic", max_results=3)
                        
                        # Sonuçları göster
                        st.markdown("### 🛒 Bulunan Satış Noktaları ve Linkler")
                        for item in stok_sonuclari.get('results', []):
                            st.markdown(f"""
                            * **[{item.get('title', 'Başlık yok')}]({item.get('url', '#')})**
                            > {item.get('content', 'Açıklama yok')}
                            """)
                        
                        st.markdown("### 🎟️ Bulunan Aktif İndirim Kodları")
                        for kupon in kupon_sonuclari.get('results', []):
                            st.markdown(f"""
                            * **[Kupon Kaynağı: {kupon.get('title', 'Başlık yok')}]({kupon.get('url', '#')})**
                            > {kupon.get('content', 'Açıklama yok')}
                            """)
                    else:
                        st.warning("Ürün tespiti yapılamadığı için arama yapılamadı.")
