...
# API Anahtarlarını Streamlit Secrets üzerinden çekiyoruz
# Kendi anahtarlarınızı Streamlit Cloud paneline girmelisiniz
GEMINI_API_KEY = st.secrets.get("GEMINI_API_KEY", "BURAYA_GEMINI_API_KEY_GELECEK")
TAVILY_API_KEY = st.secrets.get("TAVILY_API_KEY", "BURAYA_TAVILY_API_KEY_GELECEK")
    ...
    if yuklenen_fotograf is not None:
        with st.spinner("Yapay zeka görseli analiz ediyor ve canlı stok taraması yapıyor..."):
            try:
                ...
                indirim_sorgusu = f"{tespit_edilen_urun.split()} güncel indirim kodu 2026 Türkiye"
                ...
                st.error("API Anahtarları eksik veya bir hata oluştu. Lütfen ayarlarınızı kontrol edin.")
    else:
